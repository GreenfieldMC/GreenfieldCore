package com.njdaeger.greenfieldcore.commandstore.paginators;
...
public class CommandPaginator extends ChatPaginatorBuilder<SavedCommand, ICommandContext> {
    ...
    public CommandPaginator(CommandPaginatorMode mode) {
        ...
        addComponent(new PageNavigationComponent<>(
                (ctx, res, pg) -> "/" + ctx.getAlias() + " " + ctx.joinArgs().replace("-page " + pg, "") + " -page " + 1,
                (ctx, res, pg) -> "/" + ctx.getAlias() + " " + ctx.joinArgs().replace("-page " + pg, "") + " -page " + (pg - 1),
                (ctx, res, pg) -> "/" + ctx.getAlias() + " " + ctx.joinArgs().replace("-page " + pg, "") + " -page " + (pg + 1),
                (ctx, res, pg) -> "/" + ctx.getAlias() + " " + ctx.joinArgs().replace("-page " + pg, "") + " -page " + ((int) Math.ceil(res.size() / 8.0))
